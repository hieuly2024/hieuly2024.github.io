...
const util = (() => {
    ...
    const salin = (btn, msg = 'Đã sao chép', timeout = 1500) => {
...
const progress = (() => {
    ...
    const progress = () => {
        ...
        info.innerText = `Đang tải (${loaded}/${total}) [${parseInt(bar.style.width).toFixed(0)}%]`;
...
const like = (() => {
    ...
    const like = async (button) => {
        ...
        if (!token) {
            alert('Đã xảy ra lỗi, mã thông báo trống!');
            window.location.reload();
            return;
        }
...
const comment = (() => {
    ...
    const loader = `<span class="spinner-border spinner-border-sm me-1"></span>Đang tải...`;
    ...
    const innerComment = (data) => {
        ...
        <div class="d-flex flex-wrap justify-content-between align-items-center">
            <div class="d-flex flex-wrap justify-content-start align-items-center">
                ...
                ${owns.has(data.uuid)
                ? `
                <button style="font-size: 0.8rem;" onclick="comment.edit(this)" data-uuid="${data.uuid}" class="btn btn-sm btn-outline-dark rounded-3 py-0 ms-1">Thay đổi</button>
                <button style="font-size: 0.8rem;" onclick="comment.hapus(this)" data-uuid="${data.uuid}" class="btn btn-sm btn-outline-dark rounded-3 py-0 ms-1">Xoá</button>`
                : ''}
            </div>
            ...
            .then((res) => {
                if (res.code == 200) {
                    ...
                    if (res.data.length == 0) {
                        UCAPAN.innerHTML = `<div class="h6 text-center">Không có dữ liệu</div>`;
                    }
                }
            })
            .catch((err) => alert(`Có một lỗi: ${err}`));
    };
